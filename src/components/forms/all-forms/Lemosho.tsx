...
type LemoshoSchema = z.infer<typeof lemoshoRouteSchema>;
export default function LemoshoForm() {
  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm<LemoshoSchema>({
    resolver: zodResolver(lemoshoRouteSchema),
  });

  const router = useRouter();
  const { toast } = useToast();

  const { isLoading, mutateAsync } = api.lemosho.send.useMutation({
    onSuccess: () => {
  ...
  return (
    ...
    >
      ...
      >
      <h3 className="py-5  text-2xl text-white w-full bg-primary text-center">Book Your Trip</h3>
        ...
        <div className="flex flex-col items-center">
          <button
            type="submit"
            disabled={isLoading}
            className="w-[200px] rounded-lg bg-[#A87133] disabled:bg-lighter px-4 py-2 text-white hover:border hover:border-[#A87133] hover:bg-transparent hover:text-[#A87133]"
          >
            Submit Form
          </button>
        </div>
